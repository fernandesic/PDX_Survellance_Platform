...
## - intervention reduces transmission by a factor 'interv_efficacy'
## - intervention increases vaccination by an additional daily coverage equal to
##   'interv_vacc_coverage'
...
interv_delay <- user(0)
interv_efficacy <- user(0)
interv_vacc_type <- user(1)
interv_vacc_coverage <- user(0)
target_size <- user(0)
...
dim(response_vacc_coverage) <- n_populations
dim(coverage_vec) <- n_populations
...
response_vacc_coverage[] <- if (status[i] == 1 && as.integer(interv_vacc_type) == 1) interv_vacc_coverage else # global reactive vaccination
  if (status[i] == 1 && as.integer(interv_vacc_type) == 2 && S[i]>0) target_size * I[i] / S[i] else 0 # targeted reactive vaccination

coverage_vec[] <- if (status[i] == 1) vacc_coverage + response_vacc_coverage[i] else # with response
      vacc_coverage # no response
coverage_vec[] <- if (coverage_vec[i] > 1) 1 else coverage_vec[i] # avoid overshooting

p_S_V[] <- coverage_vec[i] * vacc_efficacy # prop of vaccinated; could exceed 1
