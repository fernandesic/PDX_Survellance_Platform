...
class FormLink(models.Model):
    form = models.OneToOneField(SupplierForm, on_delete=models.CASCADE, related_name='access_link')
...
class SupplierReviewer(models.Model):
    """
    Workflow reviewer assignments for the supplier-form pipeline.

    Replaces the previous WORKFLOW_REVIEWER_A_1 … D_4 environment variables.
    A reviewer is the (section, ordinal) bench position responsible for one
    of four sections (A=Contract Compliance, B=Procurement, C=Operations,
    D=Hub Lead). Each section has 1..N seated reviewers — supplier_form
    services pulls the active list ordered by `ordinal`.

    The optional `tenant` FK lets a country tenant override the global
    default bench; tenant=NULL is the global fallback.

    Edit via Django admin (no redeploy needed) — that's the whole point of
    moving this out of .env.
    """

    SECTION_CHOICES = [
        ('A', 'A — Contract Compliance'),
        ('B', 'B — Procurement Division'),
        ('C', 'C — Operations Officer'),
        ('D', 'D — Hub Lead'),
    ]

    section = models.CharField(max_length=1, choices=SECTION_CHOICES, db_index=True)
    ordinal = models.PositiveSmallIntegerField(
        help_text='Position on the bench for this section (1 = primary reviewer).',
    )
    ...
    tenant = models.ForeignKey(
        ...
        related_name='supplier_reviewers',
        help_text='Country tenant override; leave blank for the global bench.',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['section', 'is_active', 'ordinal']),
        ]
    ...
    def __str__(self):
        ...
        return f"{self.section}{self.ordinal} → {self.email}{scope}{flag}"
