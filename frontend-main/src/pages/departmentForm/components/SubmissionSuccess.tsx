...
import { DepartmentFormPdf } from './DepartmentFormPdf';
...
export const SubmissionSuccess: React.FC<SubmissionSuccessProps> = ({
    ...
    const nextToken = submissionResponse?.access_link?.token;
    const nextFillUrl = nextToken ? `${window.location.origin}/departmentForm/fill?token=${nextToken}` : '';

    useEffect(() => {
        if (nextFillUrl) {
            navigator.clipboard.writeText(nextFillUrl).catch(err => {
                logger.error('Auto-copy failed:', err);
            });
        }
    }, [nextFillUrl]);
    ...
    const handleCopyLink = () => {
        if (!nextFillUrl) return;
        navigator.clipboard.writeText(nextFillUrl);
                ...
                <div className="relative bg-white rounded-[32px] p-8 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.08)] border border-slate-100/50 text-center overflow-hidden">
                    ...
                    <div className="space-y-3 mb-8">
                        ...
                        >
                            {isSubmit ? (
                                <>
                                    Form submitted successfully! Please <strong>copy the link below</strong> and send it to
                                    <span className="text-slate-900 font-bold underline decoration-emerald-500/30 underline-offset-4 ml-1">
                                        {getNextDeptName()}
                                    </span>.
                                </>
                            ) : (
                                <>Action complete. A new link has been generated. Please <strong>copy the link below</strong> and send it back to the <span className="text-slate-900 font-bold underline decoration-rose-500/30 underline-offset-4">previous reviewer</span> so they can correct the protocol.</>
                            )}
                        </motion.p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {nextFillUrl && (
                            <motion.button
                                ...
                                className={`h-14 text-white rounded-xl font-black text-[11px] uppercase tracking-widest shadow-xl flex flex-col items-center justify-center transition-all group
                                    ${isSubmit ? 'bg-indigo-600 shadow-indigo-200' : 'bg-rose-500 shadow-rose-200 hover:bg-rose-600'}
                                `}
                            >
                                <span className="flex items-center gap-2">
                                    <Sparkles size={14} className="group-hover:animate-pulse" />
                                    {isSubmit ? 'Copy Next Stage Link' : 'Copy Revert Link'}
                                </span>
                                <span className="text-[8px] opacity-80 mt-1 lowercase font-medium tracking-normal text-white/80">
                                    {nextFillUrl.substring(0, 40)}...
                                </span>
                            </motion.button>
                        )}
                        {isSubmit && submissionResponse?.status === 'COMPLETED' && (
                            <motion.button
                                whileHover={{ scale: 1.02 }}
                                whileTap={{ scale: 0.98 }}
