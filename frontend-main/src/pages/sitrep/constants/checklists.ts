...
export type ChecklistId = '1' | '2';
export type RiskTier = 'VERY_HIGH' | 'HIGH' | 'MODERATE';
...
export const RISK_COLORS: Record<RiskTier, { bg: string; text: string; label: string }> = {
    VERY_HIGH: { bg: '#991b1b', text: '#ffffff', label: 'Very High' },
    HIGH:      { bg: '#dc2626', text: '#ffffff', label: 'High' },
    MODERATE:  { bg: '#d97706', text: '#ffffff', label: 'Moderate' },
};

// ─── Country → Risk Mapping ─────────────────────────────────────
//   Only DRC and Uganda get Checklist 1; every other AFRO Member
//   State falls through to the default (Checklist 2).

export const COUNTRY_RISK_MAP: Record<string, RiskInfo> = {
    'Democratic Republic of the Congo': { risk: 'VERY_HIGH', checklist: '1', color: '#991b1b' },
    'Uganda':                            { risk: 'HIGH',      checklist: '1', color: '#dc2626' },
};

export const DEFAULT_RISK: RiskInfo = { risk: 'MODERATE', checklist: '2', color: '#d97706' };

// ─── AFRO Member States (dropdown) ──────────────────────────────

export const AFRO_MEMBER_STATES = [
...
// ─── Pillar Options (Section 4) ─────────────────────────────────

export const PILLAR_OPTIONS = [
    'Surveillance', 'Laboratory', 'IPC', 'Case management',
    'RCCE', 'Logistics', 'Coordination', 'Border health',
];

// ─── Helper Functions ───────────────────────────────────────────

export function getCountryRisk(country: string): RiskInfo {
    return COUNTRY_RISK_MAP[country] || DEFAULT_RISK;
}

export function getChecklist(id: ChecklistId): ChecklistCategory[] {
    return id === '1' ? CHECKLIST_1 : CHECKLIST_2;
}


// ─── Checklist 1 — States with documented BDBV detection ────────
//     (DRC and Uganda)

export const CHECKLIST_1: ChecklistCategory[] = [
    {
...
export const CHECKLIST_2: ChecklistCategory[] = [
    {
