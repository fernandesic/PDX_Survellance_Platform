...
import { CheckCircle2, CircleDot, Lock, Pencil, X, Upload, ChevronDown, Paperclip, FileText, Trash2 } from 'lucide-react';
...
export const SectionCard: React.FC<SectionCardProps> = ({
    ...
    onAddAttachment, onRemoveAttachment, attachments = [],
    errors = []
}) => {
    ...
    const handleAttachFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        files.forEach(file => {
            ...
            reader.onloadend = () => {
                onAddAttachment({ name: file.name, data: reader.result as string });
            };
                        ...
                        return (
                            <div key={q.key} className="pb-2 border-b border-slate-50 last:border-0 last:pb-0 space-y-4 group">
                                ...
                                <div className="pl-12">
                                    ...
                                    {type === 'file' && null /* replaced by section-level attachments panel below */}
                                </div>
                            </div>
                        );
                    })}
                ...
                <div className="mt-6 pt-5 border-t border-slate-100">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.18em]">Supporting Documents</span>
                            {attachments.length > 0 && (
                                <span className="h-5 min-w-[20px] px-1.5 rounded-full bg-indigo-100 text-indigo-600 text-[10px] font-black flex items-center justify-center">
                                    {attachments.length}
                                </span>
                            )}
                        </div>
                        {isActive && (
                            <>
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-50 border border-indigo-100 text-indigo-600 text-[11px] font-black uppercase tracking-widest hover:bg-indigo-100 transition-all"
                                >
                    ...
                    {attachments.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-5 rounded-2xl border-2 border-dashed border-slate-100 text-slate-300">
                            <FileText className="h-6 w-6 mb-1" />
                            <p className="text-[11px] font-bold uppercase tracking-widest">
                                {isActive ? 'No attachments yet — click Add Files' : 'No attachments'}
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-2">
                            {attachments.map((file, idx) => (
                                <motion.div
                                    ...
                                    className="flex items-center gap-3 px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl group"
                                >
                                    ...
                                    <a
                                        ...
                                        className="flex-1 text-[13px] font-bold text-slate-700 truncate hover:text-indigo-600 transition-colors"
                                    >
                                        {file.name}
                                    </a>
                                    {isActive && (
                                        <button
                                            ...
                                            className="opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6 rounded-lg flex items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50"
                                        >
