...
import { SupplierFormPdf } from './components/SupplierFormPdf';
...
const AdminDashboard: React.FC = () => {
    ...
    const fetchForms = async () => {
        setIsLoading(true);
        try {
            const res = await ApiConsumer.get('/supplier/forms/');
    ...
    const fetchConfig = async () => {
        try {
            const res: any = await ApiConsumer.get('/supplier/workflow-config/');
    ...
    const handleGenerateLink = async (e: React.FormEvent) => {
        ...
        try {
            const res = await ApiConsumer.post('/supplier/generate-link/', {
                email,
                ...formFields
            });
            const token = res.access_link?.token;
            const fullLink = `${window.location.origin}/supplier?token=${token}`;
    ...
    const handleReactivate = async (id: number) => {
        try {
            await ApiConsumer.post(`/supplier/reactivate-link/${id}/`, {});
    ...
    const performDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await ApiConsumer.delete(`/supplier/delete-form/${deleteConfirm.id}/`);
    ...
    return (
        ...
        >
            <div id="supplier-form-pdf-admin" className="fixed pointer-events-none -z-50 left-[-9999px] top-0 w-[850px]" style={{ colorScheme: 'light' }}>
                {pdfData && (
                    <SupplierFormPdf
                        data={pdfData}
                        particulars={pdfData}
                        currentStatus={pdfData.status}
                    />
                )}
            </div>
            <header className="bg-[#0F172A] border-b border-white/5 relative overflow-hidden px-8 py-8 mb-4">
                ...
                <div className="max-w-7xl mx-auto flex items-center justify-between relative z-10">
                    <motion.div variants={itemVariants} className="flex items-center gap-5">
                        <img src="/assets/who-logo-white.svg" alt="WHO Logo" className="h-14 w-auto opacity-90" />
                        <div>
                        ...
                        <AnimatePresence mode="wait">
                            {isLoading ? (
                                ...
                                >
                                    ...
                                    <p className="text-slate-500 max-w-xs mx-auto mt-3 text-sm font-medium">Start by generating a secure departmental access link for a supplier.</p>
                                </motion.div>
                            ) : (
                                                    ...
                                                    <td className="px-10 py-6">
                                                        <div className="flex flex-col">
                                                            <span className="text-sm font-black text-slate-700 tracking-tight">
                                                                {link.supplier_name || 'Generic Link'}
                                                            </span>
                        ...
                        >
                            ...
                            <form onSubmit={handleGenerateLink} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Supplier Email Address <span className="text-red-500">*</span></label>
